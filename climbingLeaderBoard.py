...
def findIndex(ranked, start, end, n):
    if end - start <= 1:
        return end
    ...
    if n == ranked[mid]:
        ...
        return findIndex(ranked,mid,end,n)
    elif n > ranked[mid]:
        return findIndex(ranked,start,mid,n)
...
def climbingLeaderboard(ranked, player):
    ...
    for i in range(1,len(ranked)):
        if ranked[i] == ranked[i-1]:
            rankedOrder.append(rankedOrder[i-1])
        else:
            rankedOrder.append(rankedOrder[i-1]+1)
    rankedLastIndex = len(ranked)-1

    for i in player:
        if i > ranked[0]:
            result.append(rankedOrder[0])
            continue
        if i < ranked[rankedLastIndex]:
            result.append(rankedOrder[rankedLastIndex] + 1)
            continue
        orderIndex = findIndex(ranked,0,rankedLastIndex,i)
        result.append(rankedOrder[orderIndex])
